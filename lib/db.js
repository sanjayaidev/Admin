...
async function migrate() {
  ...
  `);
  // Defensive column backfill: sm_flow_steps may already exist in some
  // environments with an older/different shape (e.g. from
  // flow-builder-migration.sql, which used step_order/action_type/config
  // instead of order_index/action/input_map). CREATE TABLE IF NOT EXISTS
  // is a no-op against an existing table, so add any columns the rest of
  // the app relies on here, the same way it's done for `users` above.
  await pool.query(`ALTER TABLE sm_flow_steps ADD COLUMN IF NOT EXISTS order_index INTEGER NOT NULL DEFAULT 0;`);
  await pool.query(`ALTER TABLE sm_flow_steps ADD COLUMN IF NOT EXISTS module TEXT NOT NULL DEFAULT '';`);
  await pool.query(`ALTER TABLE sm_flow_steps ADD COLUMN IF NOT EXISTS action TEXT NOT NULL DEFAULT '';`);
  await pool.query(`ALTER TABLE sm_flow_steps ADD COLUMN IF NOT EXISTS connection_id UUID REFERENCES sm_connections(id);`);
  await pool.query(`ALTER TABLE sm_flow_steps ADD COLUMN IF NOT EXISTS input_map JSONB DEFAULT '{}';`);
  await pool.query(`ALTER TABLE sm_flow_steps ADD COLUMN IF NOT EXISTS condition JSONB;`);
