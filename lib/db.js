...
async function migrate() {
  ...
  // Add unique constraint on (org_id, name) if it doesn't already exist
  const constraintCheck = await pool.query(`
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'roles_org_id_name_key' AND table_name = 'roles'
  `);
  if (constraintCheck.rows.length === 0) {
    await pool.query(`ALTER TABLE roles ADD CONSTRAINT roles_org_id_name_key UNIQUE (org_id, name);`);
  }
